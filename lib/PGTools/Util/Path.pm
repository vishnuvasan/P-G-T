...
sub original_scratch_directory_path {
  ...
  if( $ENV{ PGTOOLS_HOME} ) {
    $path = if_exists( "$ENV{ PGTOOLS_HOME }/pgtools_scratch" ) 
            || if_exists( "$ENV{ PGTOOLS_HOME }/scratch" ) 
            || if_exists( $ENV{ PGTOOLS_HOME } );
  }

  $path 
    || if_exists( "$FindBin::Bin/../pgtools_scratch" ) 
    || if_exists( catfile( getcwd, 'pgtools_scratch' ) ) 
    || getcwd;
