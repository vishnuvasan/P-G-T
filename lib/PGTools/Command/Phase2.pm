...
sub run {
  ...
  # genome run will process database one single time only
  $ENV{ PGTOOLS_NO_DUPLICATE_DB } = 1;

  # possible databases
  my @databases   = grep { $config->{phase2_databases}{$_} } qw/
    utr     splice    noncode   pseudogene  
    6frame  fusion    mutation
  /;
  ...
  while( my ( $database_name, $database_path) = each( %database_paths ) ) {
    ...
    print "about to pre-prepare databases \n";

    # prepare database once, before doing anything in parallel
    if( @files > 0 ) {
      ( 
        PGTools::SearchBase->get_runnables_with_prefix(
          'PGTools::MSearch',
          $files[ 0 ],
          { dont_cleanup => 1  }
        )
    ...
    foreach my $file ( @files ) {
      ...
      my $fusion = catfile( scratch_directory_path, file_without_extension( $file ), 'fusion.csv' ); 


      # fix outputs where they need to be
      if( $database_name =~ /splice|mutation|fusion/ && $no_options_given ) {

        # overlap
        push @to_run, sub {

          mv $merge, "$merge.original";

          run_pgtool_command " fix_output -d $database_path -i " 
            . " $merge.original -o $merge " 
            . " --overlap=@{[ $config->{junction_overlap} || 3 ]} ";

        };

      }

      elsif( $database_name =~ /utr|pseudo|noncode/ && $no_options_given ) {
        push @to_run, sub {
          mv $merge, "$merge.original";
      ...
      push @to_run, sub {
        ...
        # Run fusion as part of phase2
        if( $name =~ /fusion/ ) {
          if( ! $compute_full_runs && ( $no_options_given || $options->{fusion} ) ) {
            run_pgtool_command " post_process --fusion --input=$merge --output=$fusion ";
          }
        }
    ...
    if( $compute_full_runs ) {
      ...
      my $fusion          = catfile( $hold_directory, 'fusion.csv' );
      ...
      # run merge to bed
      run_pgtool_command " merge2bed --merge=$output_file --bed=$bed_file --database=$name --color=red"
        if $name !~ /fusion|mutation/; 
      ...
      if( $database_name =~ /fusion/ ) {
        if( $no_options_given || $options->{fusion} ) {
          run_pgtool_command " post_process --fusion --input=$output_file --output=$fusion";
        }
      }
    }
