...
use PGTools::Command::Visualize::Decoy;
use PGTools::Command::Visualize::SplatteredBar;
...
sub run  {
  ...
  # run
  for my $type ( qw/ circos venn bar decoy splatteredbar/ ) {
    if( $options->{ $type } ) {
      _run( $type, $options );
      return;
    }
...
sub _run {
  ...
  eval {
    ( 'PGTools::Command::Visualize::' . ( $class_map{ $class } || ucfirst( $class ) ) )->run( $options );
  };
