...
sub bed_entry_from_row {
  ...
  my ( $start_pos_2, $end_pos_2 );

  my ( $thick_start, $thick_end, $block_count, $block_size, $block_start ) = ( 0, 0, 0, 0, 0 );
  ...
  if( 
    ...
    ( $start_pos, $end_pos, $strand, $chromosome, $start_pos_2, $end_pos_2 ) 
      = @matches{ qw/start_pos end_pos strand chromosome start_pos_2 end_pos_2/ };
    ...
    # Add chr if required
    $chromosome = 'chr' . $chromosome if $chromosome =~ /^(\d+|[XY])$/i;

    if( $start_pos_2 && $end_pos_2 ) {
      $block_count = 2;
      $block_size = join( ',', $end_pos - $start_pos, $end_pos_2 - $start_pos_2 );
      $block_start = join( ',', 0, $start_pos_2 - $start_pos ); 
    }
  ...
  [ 
    $chromosome, $start_pos, $end_pos, $database, 0, 
    $strand, $thick_start, $thick_end, $color, 
