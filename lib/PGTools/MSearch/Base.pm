...
sub no_duplicate_db {
  $ENV{ PGTOOLS_NO_DUPLICATE_DB };
}

sub db_save_path {
  my $path = PGTools::Configuration->new->config->{database_save_path} || "/tmp/pgtools/database";
...
sub database {
  ...
  return $self->msearch_config->{database} 
    unless $self->no_duplicate_db;

  my $db = $self->msearch_config->{database};
  my $ndb = catfile( $self->db_save_path, file( $db ) );

  cp $db, $ndb unless -e $ndb; 
...
sub use_directory_from_env {
  $ENV{ PGTOOLS_USE_SYMLINKED_DATABASE } && 
  $ENV{ PGTOOLS_CURRENT_RUN_DIRECTORY  } && 
  -d $ENV{ PGTOOLS_CURRENT_RUN_DIRECTORY } 
}
...
sub database_path_for {
  ...
  unless( $self->no_duplicate_db ) {
    ...
    if( $self->use_directory_from_env ) {
    ...
    my $path = catfile( $self->db_save_path, 'decoy' );
